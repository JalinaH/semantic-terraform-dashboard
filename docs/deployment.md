...
docker buildx build \
  ...
  --tag <account>.dkr.ecr.<region>.amazonaws.com/semantic-terraform-worker:1.2.0-amd64 \
  --push \
  .
```
...
| `AWS_CONTROL_PLANE_REGION` | no | STS client region; required for Cloud Verification |
| `AWS_ASSUME_ROLE_PRINCIPAL_ARN` | no | shared identity; required for Cloud Verification/onboarding |
...
| `SEMANTIC_TERRAFORM_AGENT_VERSION` | no | defaults to and must equal `1.2.0` |
...
polling. The image installs the exact v1.2.0 commit supplied through the
required `SEMANTIC_TERRAFORM_AGENT_SOURCE` build argument; the Dockerfile has no
mutable default. `importlib.metadata` is the authoritative check at image build
and worker startup.
...
8. Worker starts with `agentVersion: 1.2.0` and atomically claims diagnosis or patch-application jobs.
