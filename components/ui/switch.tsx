...
export function Switch({ defaultChecked = false, disabled = false, label, className, ...props }: SwitchProps) {
  return (
    <label className={cn("relative inline-flex items-center", disabled && "cursor-not-allowed opacity-50", className)}>
      <input {...props} type="checkbox" role="switch" aria-label={label} defaultChecked={defaultChecked} disabled={disabled} className="peer sr-only" />
