...
export function ApplyVerifiedPatch({ run }: { run: {
  ...
  const localConditional = conditional && run.verificationOutcome === "locally_validated";
      ...
      <Card className={conditional ? "border-warning/25" : "border-success/25"}>
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><CardTitle>{localConditional ? "Locally validated patch" : conditional ? "Patch validated, full plan blocked" : "Verified patch"}</CardTitle><CardDescription className="mt-1">{localConditional ? "Patch check, patch apply, fmt, init, and validate passed. Provider-aware Terraform plan was not requested." : conditional ? "Patch safety, fmt, init, and validate passed; Terraform plan was blocked by an external condition." : `All isolated Terraform verification stages passed, including plan, against ${truncateSha(run.verifiedAgainstCommitSha!)}.`}</CardDescription></div>
            <Badge className={conditional ? "border-warning/25 bg-warning-muted text-warning-foreground" : "border-success/25 bg-success-muted text-success-foreground"} variant="outline">{conditional ? <ShieldAlert aria-hidden="true" /> : <CheckCircle2 aria-hidden="true" />}{localConditional ? "Locally validated" : conditional ? "Environment blocked" : "Fully verified"}</Badge>
          </div>
        </CardHeader>
        <CardContent className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div className="text-xs leading-5 text-muted-foreground">{localConditional ? <><p>Cloud verification is not configured for this repository.</p><p className="font-medium text-warning-foreground">Review the diff; repository CI should perform final provider-aware validation.</p></> : conditional ? <><p>{planFailureClassLabel(run.planFailure?.classification ?? null)}: {run.planFailure?.summary ?? "external condition reported"}.</p><p className="font-medium text-warning-foreground">This patch has not passed a complete Terraform plan.</p></> : <><p>Review the suggested diff before applying it.</p><p>Terraform verification does not establish developer intent.</p></>}</div>
          <Button onClick={() => setOpen(true)}>{localConditional ? "Review & Apply" : conditional ? "Apply validated patch" : "Apply to PR"}</Button>
        </CardContent>
      </Card>
      {open && !state.ok ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4 backdrop-blur-sm" role="presentation">
          <div role="dialog" aria-modal="true" aria-labelledby="apply-dialog-title" className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl border bg-card shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b p-5">
              <div><h2 id="apply-dialog-title" className="font-semibold">{localConditional ? "Apply locally validated patch?" : conditional ? "Apply conditionally verified fix?" : "Apply verified fix"}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Explicitly authorize one TerraFix bot commit to this same-repository PR branch.</p></div>
...
function availabilityMessage(run: { mutationEligible: boolean | null; mutationEligibilityReason: string | null; pullRequestNumber: number | null; verifiedAgainstCommitSha: string | null; verificationStatus: RunVerificationStatus; verificationOutcome: VerificationOutcome | null; planFailure: { summary: string } | null }) {
  ...
  if (run.verificationStatus !== "verified_first_attempt" && run.verificationStatus !== "verified_after_retry" && run.verificationStatus !== "locally_validated_first_attempt" && run.verificationStatus !== "locally_validated_after_retry") return "Only successfully verified patches can be applied.";
