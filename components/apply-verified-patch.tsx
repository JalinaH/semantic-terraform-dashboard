...
export function ApplyVerifiedPatch({ run }: { run: {
  ...
  const verified = run.verificationStatus === "verified_first_attempt" || run.verificationStatus === "verified_after_retry";
  const canOffer = verified && run.mutationEligible === true && Boolean(run.pullRequestNumber && run.patchSha256 && run.verifiedAgainstCommitSha && run.patchAffectedFiles.length) && !latest;
      ...
      <Card className="border-success/25">
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><CardTitle>Verified patch</CardTitle><CardDescription className="mt-1">Verified against {truncateSha(run.verifiedAgainstCommitSha!)} with a reproducible v1.1 artifact.</CardDescription></div>
            <Badge className="border-success/25 bg-success-muted text-success-foreground" variant="outline"><CheckCircle2 aria-hidden="true" />Verified</Badge>
          </div>
        </CardHeader>
        <CardContent className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div className="text-xs leading-5 text-muted-foreground"><p>Review the suggested diff before applying it.</p><p>Terraform verification does not establish developer intent.</p></div>
          <Button onClick={() => setOpen(true)}>Apply to PR</Button>
        </CardContent>
      </Card>
      {open && !state.ok ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4 backdrop-blur-sm" role="presentation">
          <div role="dialog" aria-modal="true" aria-labelledby="apply-dialog-title" className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl border bg-card shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b p-5">
              <div><h2 id="apply-dialog-title" className="font-semibold">Apply verified fix</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Explicitly authorize one TerraFix bot commit to this same-repository PR branch.</p></div>
            ...
            <form action={action} className="space-y-5 p-5">
              ...
              <div className="rounded-lg border border-warning/25 bg-warning-muted p-4 text-xs leading-5 text-warning-foreground">
                <ShieldAlert aria-hidden="true" className="mr-1.5 inline size-4" />TerraFix will recheck the PR head, reapply the exact verified patch in a temporary checkout, rerun safe Terraform verification, then push one non-force commit. It will not merge the PR or run Terraform apply.
              </div>
              {state.message && !state.ok ? <p role="alert" className="text-xs text-danger-foreground">{state.message}</p> : null}
              <div className="flex justify-end gap-2"><Button type="button" variant="outline" disabled={pending} onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={pending}>{pending ? <><LoaderCircle aria-hidden="true" className="animate-spin" />Checking and queuing…</> : "Apply verified fix"}</Button></div>
            </form>
          </div>
        </div>
      ) : null}
...
function availabilityMessage(run: { mutationEligible: boolean | null; mutationEligibilityReason: string | null; pullRequestNumber: number | null; verifiedAgainstCommitSha: string | null; verificationStatus: RunVerificationStatus }) {
