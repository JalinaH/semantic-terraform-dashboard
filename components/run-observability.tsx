...
function VerificationAssessment({ run }: { run: RunDetail }) {
  ...
  const explanation = run.verificationOutcome === "fully_verified"
    ...
    : run.verificationOutcome === "locally_validated"
      ? "TerraFix successfully validated the patch locally. Provider-aware plan verification was not requested because cloud verification is not configured."
