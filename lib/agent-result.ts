...
const attemptSchema = z.object({
  ...
  verification_mode: z.enum(["local", "full"]).default("full"),
  plan_requested: z.boolean().default(true),
  plan_skip_reason: z.literal("cloud_verification_not_configured").nullable().optional(),
}).passthrough();
...
const verificationProvenanceSchema = z.object({
  ...
  plan_passed: z.boolean().nullable(),
  plan_skip_reason: z.literal("cloud_verification_not_configured").nullable().optional(),
...
const verificationAssessmentSchema = z.object({
  outcome: z.enum(["fully_verified", "locally_validated", "environment_blocked", "semantic_failure", "patch_invalid", "unknown_failure"]),
  verification_mode: z.enum(["local", "full"]).default("full"),
  plan_requested: z.boolean().default(true),
  patch_check_passed: z.boolean(),
  patch_apply_passed: z.boolean(),
...
const successfulResultSchema = z.object({
  ...
  verification_mode: z.enum(["local", "full"]).nullable().optional(),
  plan_requested: z.boolean().nullable().optional(),
  plan_attempted: z.boolean().nullable().optional(),
  plan_skip_reason: z.literal("cloud_verification_not_configured").nullable().optional(),
...
export function sanitizeSuccessfulAgentResult(result: SuccessfulAgentResult, packageVersion?: string) {
  ...
  const attempts = result.diagnosis.attempts.map((attempt) => ({
    ...
    verificationMode: attempt.verification_mode,
    planRequested: attempt.plan_requested,
    planSkipReason: attempt.plan_skip_reason ?? null,
  }));
  ...
  const telemetry = {
    ...
    verificationMode: verificationAssessment?.verification_mode ?? result.verification_mode ?? null,
    ...
    assessmentPlanRequested: verificationAssessment?.plan_requested ?? result.plan_requested ?? null,
    assessmentPlanPassed: verificationAssessment?.plan_passed ?? null,
    planSkipReason: verificationAssessment?.plan_skip_reason ?? result.plan_skip_reason ?? null,
  ...
  const safeResultPayload = {
    ...
    verificationProvenance: verificationProvenance ? {
      verificationMode: verificationProvenance.verification_mode,
      ...
      planRequested: verificationProvenance.plan_requested,
      planAttempted: verificationProvenance.plan_attempted ?? null,
      planPassed: verificationProvenance.plan_passed,
      planSkipReason: verificationProvenance.plan_skip_reason,
    ...
    verificationAssessment: verificationAssessment ? {
      verificationMode: telemetry.verificationMode,
