...
const verificationStatusToDatabase: Record<SuccessfulAgentResult["diagnosis"]["verification_status"], VerificationStatus> = {
  ...
  locally_validated_first_attempt: VerificationStatus.LOCALLY_VALIDATED_FIRST_ATTEMPT,
  locally_validated_after_retry: VerificationStatus.LOCALLY_VALIDATED_AFTER_RETRY,
    ...
    await db.$transaction(async (transaction) => {
      const completed = await transaction.agentRun.updateMany({
        ...
        data: {
          ...
          verificationMode: result.telemetry.verificationMode,
          ...
          assessmentPlanRequested: result.telemetry.assessmentPlanRequested,
          assessmentPlanPassed: result.telemetry.assessmentPlanPassed,
          planSkipReason: result.telemetry.planSkipReason,
