      ...
      <Card className="overflow-hidden">
        <CardContent className="p-0">
          ...
          <SettingsSection icon={ShieldCheck} title="TerraFix Access" description="Server-enforced model access policy for this account.">
            <dl className="grid gap-2 text-xs sm:grid-cols-2"><div className="rounded-lg border p-3"><dt className="text-muted-foreground">Current access</dt><dd className="mt-1 font-medium">{formatAccess(catalog.access.accessLevel)}</dd></div><div className="rounded-lg border p-3"><dt className="text-muted-foreground">Maximum model tier</dt><dd className="mt-1 font-mono font-medium">{catalog.access.maximumTier}</dd></div><div className="rounded-lg border p-3"><dt className="text-muted-foreground">Available models</dt><dd className="mt-1 font-medium">{catalog.models.filter((model) => model.allowed).length}</dd></div><div className="rounded-lg border p-3"><dt className="text-muted-foreground">Catalog sync</dt><dd className="mt-1 font-medium">{catalog.sync?.lastSuccessfulAt ? catalog.sync.lastSuccessfulAt.toLocaleString() : "Not synchronized"}</dd></div><p className="col-span-full rounded-lg bg-secondary/30 p-3 leading-5 text-muted-foreground">This deployment currently enables free model access only. No checkout or subscription is created.</p></dl>
          </SettingsSection>

          <SettingsSection icon={Cloud} title="Cloud Verification" description="Optional repository-scoped AWS access for provider-aware Terraform plan verification.">
            <div className="space-y-3">
              ...
              <div className="rounded-lg border bg-secondary/30 p-3 text-xs leading-5 text-muted-foreground"><p><span className="font-medium text-foreground">AWS is optional.</span> TerraFix works without AWS using isolated local validation. Connected repositories use a repository-specific IAM role with temporary STS credentials for provider-aware plan verification; permanent access keys are never requested.</p></div>
